...
Roadmap
的目的不是自行重新設計產品，而是依照正確依賴關係逐步建立已確認的產品架構。
...
8.  OCR 結果經 Coordinate Transformation（必要時）後，再交給 Hotspot
    Generator。
...
Reading Mode 提供單一 Edit Button（編輯按鈕）作為 Editor
Mode（編輯模式）入口。

使用者透過 Edit Button 進入既有 Editor，不建立第二套 Editor
流程、資料模型或編輯架構。
...
本版本僅同步既有已確認產品規格，不新增產品功能、不新增資料模型、不擴大
MVP Scope。

同步內容：

1.  Reading Mode → 單一 Edit Button → Editor Mode 入口，與 01 Product
    Specification v3.3、08 Editor Design v1.1 一致。
2.  AI Automation 中 Coordinate Transformation（必要時）移至 Hotspot
    Generator 之前，與 05 Database Design、06 API Design、07 AI Design
    及 08 Editor Design 的已確認流程一致。
